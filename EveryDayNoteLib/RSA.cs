...
namespace EveryDayNoteLib
{
    class RSA
    {
        ...
        private void InitKeyData()
        {
            ...
            this.n = (ushort)(this.p * this.q); this.phi = (ushort)((p - 1) * (q - 1));
        ...
        { return this.n; }
        public Int32 GetDKey() { return this.d; }
        ...
        {
            string outStr = ""; Int32 n = Int32.Parse(n_s); Int32 d = Int32.Parse(d_s);
            Int32[] arr = GetDecArrayFromText(text); byte[] bytes = new byte[arr.Length];
            System.Text.UTF8Encoding enc = new System.Text.UTF8Encoding();
            int j = 0; foreach (int i in arr)
            {
                byte decryptedValue = (byte)ModuloPow(i, d, n);
                bytes[j] = decryptedValue; j++;
            }
        ...
        {
            int i = 0; foreach (char c in text)
            { if (c == '|') { i++; } } Int32[] result = new Int32[i]; i = 0; string tmp = "";
            foreach (char c in text)
            { if (c != '|') { tmp += c; } else { result[i] = Int32.Parse(tmp); i++; tmp = ""; } } return result;
        }
        ...
        {
            int u1 = 1; int u3 = a; int v1 = 0; int v3 = b;
            while (v3 > 0)
            {
                int q0 = u3 / v3; int q1 = u3 % v3; int tmp = v1 * q0; int tn = u1 - tmp;
                u1 = v1; v1 = tn; u3 = v3; v3 = q1;
            } 
            int tmp2 = u1 * (a); 
            tmp2 = u3 - (tmp2); 
            int res = tmp2 / (b);
            ExtendedEuclideanResult result = new ExtendedEuclideanResult() { u1 = u1, u2 = res, gcd = u3 }; return result;
        }
        static private byte[] GetNotDivideable()
        {
            ...
            for (int x = 2; x < 256; x++)
            {
                int n = 0; for (int y = 1; y <= x; y++)
                { if (x % y == 0) n++; } if (n <= 2) notDivideable.Add((byte)x);
            }
